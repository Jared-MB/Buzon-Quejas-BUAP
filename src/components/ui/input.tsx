...
export function InputContainer({
	...
	return (
		<div
			className={clsx("grid w-full items-center gap-2", classNames?.container)}
		>
			{children}
		</div>
	);
...
export function InputError({ children, id }: { children?: React.ReactNode, id: string }) {
	return <small id={id} className="text-rose-400 text-xs hidden">{children}</small>;
}
